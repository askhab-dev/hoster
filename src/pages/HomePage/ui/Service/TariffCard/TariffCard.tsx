...
export const TariffCard = (props: TariffCardProps) => {
  const { isPopular = false, isAvailable = true, name, description, cpu, ram, storage, traffic, oldPrice = null, price } = props;

  return (
    <div className={styles.container}>
      {isPopular && <p className={styles.popularText}><StarIcon className={styles.star} />САМЫЙ ПОПУЛЯРНЫЙ</p>}
      <div className={cx(styles.cardWrapper, isPopular && styles.popular, !isAvailable && styles.disabled)}>
        ...
        <div className={styles.cardFooter}>
          <div className={styles.priceWrapper}>
            ...
            <p className={styles.price}>${price}<span className={styles.month}>/месяц</span></p>

          </div>
          <Button className={styles.button} variant='primary' disabled={!isAvailable}>{isAvailable ? 'Купить' : 'Нет в наличии'}</Button>
        </div>
      </div>
      </div>
    </div>
